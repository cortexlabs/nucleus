...
class FileBasedModelsTreeUpdater(mp.Process):
    ...
    def __init__(
        ...
        Args:
            ...
            model_server_config: The model server config as passed on by the user using the CLI.
...
class TFSModelLoader(mp.Process):
    ...
    def _determine_model_signature_key(self, model_name: str) -> Optional[str]:
        if self._models_dir:
            signature_key = self._model_server_config["multi_model_reloading"]["signature_key"]
        else:
